...
/// A single message in a conversation, sent by a user, assistant, system, or tool.
///
/// Messages form the conversation history passed to LLM providers. Each message
/// can optionally include tool call requests/responses and image attachments for
...
///
/// Maps to the standard chat-completion role field used by OpenAI-compatible
/// and Anthropic APIs.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum Role {
    /// System prompt providing instructions and context to the model.
    System,
    /// Message from the human user.
    User,
    /// Response generated by the LLM.
    Assistant,
    /// Output from a tool execution, paired with a `tool_call_id`.
    Tool,
}

/// A request from the LLM to invoke a named tool with JSON arguments.
///
/// The `id` field correlates the call with its corresponding [`Message`] of
/// role [`Role::Tool`] that carries the result.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ToolCall {
    pub id: String,
    pub name: String,
...
/// JSON Schema definition for a tool that the LLM may invoke.
///
/// Passed alongside messages so the model knows which tools are available and
/// how to format their arguments.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Response from an LLM provider, containing either text or tool-call requests.
///
/// Wraps the response content together with optional token usage statistics for
...
/// The payload of an LLM response — either plain text or a set of tool calls.
pub enum LLMResponseContent {
    /// Free-form text response.
    Text(String),
    /// One or more tool invocations the model wants to execute.
    ToolCalls {
...
/// An incremental chunk received during a streaming LLM response.
///
/// Consumers append `delta` text as chunks arrive. When `done` is `true`, the
/// stream is complete and any accumulated `tool_calls` are available.
#[derive(Debug, Clone)]
